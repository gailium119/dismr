...
int sha256_calcufile(const wchar_t* pfile, unsigned char* psha256_value, int nvalue_length)
{
	...
	if (nvalue_length < 32)
	{
		printf("value length error\n");
		return -1;
	}

	memset(abuf, 0, SIZE);
	/* 打开文件 */
	_wfopen_s(&fp,pfile, L"rb");
	if (fp == NULL) {
		wprintf(L"Can't open \"%ls\"", pfile);
		return -1;
	}
	/* 初始化指针 */
	sha256_init(&ctx);
	...
	while (1)
	{
		...
		sha256_update(&ctx, (const unsigned char*)abuf, nlen);
	}
	/*生成哈希值 */
	sha256_final(&ctx, psha256_value);
	fclose(fp);
	return 0;
}
